...
class AwsDeployFunction {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'deploy:function:initialize': async () => {
        ...
        progress.get('main').notice('Validating');
        log.info();
        log.info('Validating');
        await this.validate();
  ...
  async callUpdateFunctionConfiguration(params) {
    ...
    await callWithRetry();
    legacy.log(`Successfully updated function: ${this.options.function}`);
  }
  ...
  async updateFunctionConfiguration() {
    ...
    if (!Object.keys(_.omit(params, 'FunctionName')).length) {
      legacy.log('Configuration did not change. Skipping function configuration update.');
      if (this.options['update-config']) {
        log.notice();
      }
      log.notice(
    ...
    legacy.log('Updating function configuration');
    progress.get('main').notice('Updating function configuration');
    log.info('Updating function configuration');
    await this.callUpdateFunctionConfiguration(params);
    log.notice(
      `${style.noticeSymbol('✔')} Function configuration updated ${style.aside(
  ...
  async deployFunction() {
    ...
    if (functionObject.image) {
      ...
      if (remoteImageSha === functionImageSha && !this.options.force) {
        ...
        legacy.log('Image did not change. Skipping function deployment.');
        return;
      }
      params.ImageUri = functionImageUri;
      progress.get('main').notice('Uploading');
      log.info('Uploading');
    } else {
      ...
      if (remoteHash === localHash && !this.options.force) {
        legacy.log('Code not changed. Skipping function deployment.');
        log.notice(
          `\n${style.noticeSymbol(
            '∅'
      ...
      legacy.log(`Uploading function: ${this.options.function} (${filesize(stats.size)})...`);
      progress.get('main').notice(`Uploading ${style.aside(`(${filesize(stats.size)})`)}`);
      log.info(`Uploading ${style.aside(`(${filesize(stats.size)})`)}`);
    }
    ...
    log.notice();
    log.notice(
      `${style.noticeSymbol('✔')} Function code deployed ${style.aside(
        `(${Math.floor((Date.now() - this.serverless.pluginManager.commandRunStartTime) / 1000)}s)`
      )}`
    );
  }
